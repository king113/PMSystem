...
var cities = {
    "recommend":["北京市"],
    "host": ["上海市","成都市","杭州市","深圳市","郑州市","重庆市","南京市","广州市"]
};
(function ($, win, doc) {
    ...
    p.createProList = function () {
        ...
        var tuijian = "";
        var remen = "";
        for(var i=0; i<cities.recommend.length; i++){
            tuijian+='<span>'+cities.recommend[i]+'</span>';
        }
        for(var j=0; j<cities.host.length; j++){
            remen+='<span>'+cities.host[j]+'</span>';
        }
        //console.log(remen);


        proBox = "<section class='pro-picker'><dl class='recommend common'><a>推荐城市</a>"+ tuijian +"</dl><dl class='host common'><a>热门城市</a>"+ remen +"</dl>" + dl + "</section>";

        $(".picker-box").append(proBox);

        //热门城市和推荐城市点击输出
        $(".common").children("span").on("click", function(e){
            var that = this;
            var tag = e.toElement;
            if ($(tag).is("span")) {
                that.city = $(tag).html();
                $(".cities").val(that.city);//市区
                $(".picker-box").hide();
                window.location.href="ad_indexlph?city="+that.city;
                layer.open({
                    content: '当前城市为：'+that.city
                    ,skin: 'msg'
    ...
    p.cityClick = function () {
        ...
        $(".city-picker").on("click", function (e) {
            ...
            if ($(target).is("li")) {
                ...
                if (that.elType) {
                    //that.el.val(that.pro + "-" + that.city);//省+市区
                    that.el.val(that.city);//市区
                    window.location.href="ad_indexlph?city="+that.city;
                    layer.open({
                        content: '当前城市为：'+that.city
                        ,skin: 'msg'
                    ...
                    //that.el.html(that.pro + "-" + that.city);//省+市区
                    that.el.html(that.city);//市区
                    window.location.href="ad_indexlph?city="+that.city;
                    layer.open({
                        content: '当前城市为：'+that.city
                        ,skin: 'msg'
